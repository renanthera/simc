...
// /* TODO:
//      Effect Categories:
//       - (from data_enums.hh:spell_attribute)
//         * SX_NO_DODGE
//         * SX_NO_PARRY
//         * SX_NO_BLOCK
//         * SX_NO_MISS
// i       * SX_CANNOT_CRIT
//         * SX_TREAT_AS_PERIODIC
//         * Pierce Immunities
//       - (not modeled in codebase)
//         * WHITE_MELEE // triggers effects like EB, ZM cancel, Earthwarden
//      Miss chance:
// i      - player_chance=k -> chance to occur on each occurrence
//      Trigger on multiple %:
//        - Doable with multiple raid events and tightly-packed first_pct->last_pct, but inconvenient
//        - trigger_pct=t/u/v -> enqueued when actor reaches %, cooldown begins, occurs one (or count=k) times
//      Psuedo-Channeling:
//        - count=k -> go on cd and perform k events (equivalent to duration, but more convenient)
//      Multi-Part Abilities:
//       - amount=x/y,type=physical/holy
//         * supports most fields, standard opt-list separator syntax
//      Damage Source and Target:
// i      - source_name="name_str" -> source matches name
//        - source_match="regex" -> source matches regex, duplicated per match
//        - target_name="name_str" -> target matches name
//        - target_match="regex" -> target matches regex, aoes all matches (affected_players)
// i      - target_role="role_string" -> target matches role, aoes all matches (affected_players affected_role)
//      Callbacks to trigger dot/debuff on result
//        - debuff stacking direction (stacks up or stacks down)
//        - debuff action (damage amp? dot?)
